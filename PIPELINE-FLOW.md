...
1. **Name matching** — regex patterns (e.g. `/^button\b|btn\b|cta\b/i` → `button`)
2. **Visual heuristics** — layout/dimensions (e.g. h≤64, horizontal, 1-3 children, has text → `button`)
3. **Component properties** — if INSTANCE has `componentProperties` from a COMPONENT_SET (e.g. `Type=Primary, Disabled=No`), infers formRole from property keys/values
...
| `src/compile/component-gen.ts` | PATH C sub-pipeline — discovery, generation, substitution |
...
| `src/compile/retry.ts` | LLM → Mitosis parse → retry loop |
| `src/shadcn/shadcn-codegen.ts` | shadcn generation for all 3 paths |
| `src/shadcn/shadcn-prompt-builder.ts` | All shadcn LLM prompts |
...
| `src/shadcn/shadcn-types.ts` | formRole → shadcn type mapping |
...
| `src/prompt/assemble.ts` | Non-shadcn prompt assembly |
